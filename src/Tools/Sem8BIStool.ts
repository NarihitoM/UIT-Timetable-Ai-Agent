    ...
    async (input) => {
        try {
            ...
            if (!fs.existsSync(filePath)) {
                return `Error: The file "Sem8_BIS.txt" could not be found in the data/ directory.`;
            }
            ...
            if (input.query && input.query.trim() !== "") {
                ...
                if (matchedLines.length === 0) {
                    return `File "Sem8_BIS.txt" read successfully. No specific matching records found for search term: "${input.query}".`;
                }
                return `File "Sem8_BIS.txt" read successfully. Filtered matches for "${input.query}":\n\n${matchedLines.join("\n")}`;
            }

            return `Successfully read Sem8_BIS.txt data contents:\n\n${fileContent}`;

        } catch (error) {
            return `Failed to read Sem8_BIS.txt. Error details: ${String(error)}`;
        }
    },
    {
