...
export const readSectionATool = tool(
    async (input) => {
        try {
            const filePath = path.resolve(process.cwd(), "src" ,"data", "Section A.txt");
    ...
    },
    {
        ...
        schema: z.object({
            query: z.string().optional().describe("An optional keyword or search term (like a class name, subject, or code) to filter relevant lines from Section A.txt.")
        })
    }
);